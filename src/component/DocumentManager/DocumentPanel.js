...
// ✅ 可排序的文档项 - 整个项目都可拖拽
const SortableDocumentItem = SortableElement(({ document, isActive, onClick, onRename, onDelete, isRenaming, tempName, onTempNameChange, onRenameConfirm, onRenameCancel, onMouseDown }) => {
...
// ✅ 可排序的文件夹项 - 整个项目都可拖拽
const SortableFolderItem = SortableElement(observer(({ folder, isExpanded, onToggle, onRename, onDelete, onCreateDocument, onCreateFolder, children, isRenaming, tempName, onTempNameChange, onRenameConfirm, onRenameCancel, onMouseDown }) => {
  ...
  return (
    <div className={`folder-item sortable-item ${isExpanded ? 'expanded' : ''}`}>
      <Dropdown
        ...
        disabled={isRenaming}
      >
        <div className="folder-header" onClick={handleFolderClick} onMouseDown={handleMouseDown} style={{ cursor: isRenaming ? 'default' : 'pointer' }}>
          <div className="item-content" style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
            <span
...
const SortableDocumentsList = SortableContainer(observer(({ items, folderId, documentManager, onDocumentClick, onRename, onDelete, renamingItem, onTempNameChange, onRenameConfirm, onRenameCancel, onMouseDown }) => {
  return (
...
// ✅ 可排序的文件夹列表容器
const SortableFoldersList = SortableContainer(observer(({ items, parentId, documentManager, onFolderToggle, onDocumentClick, onRename, onDelete, onCreateDocument, onCreateFolder, buildFolderContent, renamingItem, onTempNameChange, onRenameConfirm, onRenameCancel, onMouseDown }) => {
  return (
    ...
    setTimeout(() => {
      ...
      if (helper && !helper.querySelector('.helper-content')) {
        ...
        if (this.lastMouseDownEvent) {
          ...
          mouseOffset = {
            x: this.lastMouseDownEvent.clientX - nodeRect.left,
            y: this.lastMouseDownEvent.clientY - nodeRect.top
          };

          // 确保偏移量在合理范围内
          mouseOffset.x = Math.max(0, Math.min(mouseOffset.x, nodeRect.width));
          mouseOffset.y = Math.max(0, Math.min(mouseOffset.y, nodeRect.height));
        } else {
          // 如果没有记录到鼠标按下事件，使用元素中心作为偏移
          const nodeRect = node.getBoundingClientRect();
          mouseOffset = {
            x: nodeRect.width / 2,
            y: nodeRect.height / 2
        ...
        const handleMouseMove = (e) => {
          if (helper && helper.parentNode) {
            // 使用记录的偏移量来定位helper，让其保持在鼠标相对于原元素的位置
            helper.style.left = (e.clientX - mouseOffset.x) + 'px';
            helper.style.top = (e.clientY - mouseOffset.y) + 'px';
          }
        };
        ...
        {folders.length > 0 && (
          <div className="folders-section">
            <SortableFoldersList
              ...
              getHelperDimensions={({ node }) => ({ width: 240, height: 36 })}
            />
          </div>
        )}
