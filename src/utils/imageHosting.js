...
function writeToEditor({ content, image }) {
  ...
  // 确保编辑器内容同步到store中，用于剪贴板上传和直接拖拽上传
  try {
...
export const localImageUpload = async ({
  ...
  try {
    ...
    const response = await axios.post(
      `${baseURL}/api/upload-image/${userId}`,
      formData,
      {
        headers: {
    ...
    hideUploadNoti();
    console.error('本地图床上传失败:', error);
    uploadError(error.response?.data?.message || error.message || '本地图床上传失败');
...
export const uploadAdaptor = (...args) => {
  const type = localStorage.getItem(IMAGE_HOSTING_TYPE); // 本地图床 | SM.MS | 阿里云 | 七牛云 | Gitee | GitHub | 用户自定义图床
  const userType = imageHosting.hostingName;

  // 如果没有配置图床类型，默认使用本地图床
  if (!type) {
    localStorage.setItem(IMAGE_HOSTING_TYPE, IMAGE_HOSTING_NAMES.local);
    return localImageUpload(...args);
  }

  if (type === IMAGE_HOSTING_NAMES.local) {
    return localImageUpload(...args);
    ...
    const config = JSON.parse(window.localStorage.getItem(GITEE_IMAGE_HOSTING));
    if (!config || !config.username?.length || !config.repo?.length || !config.token?.length) {
